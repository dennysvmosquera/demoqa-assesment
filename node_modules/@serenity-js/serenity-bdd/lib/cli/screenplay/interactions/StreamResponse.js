...
class StreamResponse extends core_1.Interaction {
    ...
    performAs(actor) {
        return actor.answer(this.request)
            ...
            const totalBytes = parseInt(response.headers['content-length'], 10), output = abilities_1.UseFileSystem.as(actor).createWriteStreamTo(this.destination);
     ...
     */
    toString() {
        return io_1.formatted `#actor saves response to ${this.request} in ${this.destination.value}`;
    }
}
