...
class SerenityReporterForMocha extends mocha_1.reporters.Base {
    ...
    announceSuiteStartsFor(suite) {
        ...
        const details = new model_1.TestSuiteDetails(new model_1.Name(suite.title), new io_1.FileSystemLocation(io_1.Path.from(suite.file)), // all suites except for the root suite should have .file property set
        suiteId);
    ...
    announceSceneFinishedFor(test, runnable) {
        ...
        this.emit(new events_1.SceneFinishes(this.currentSceneId, outcome, this.serenity.currentTime()));
        return this.serenity.waitForNextCue()
            ...
            const errorOutcome = new model_1.ExecutionFailedWithError(error);
            this.emit(new events_1.SceneFinished(this.currentSceneId, scenarioDetails, errorOutcome.isWorseThan(outcome)
                ? errorOutcome
    ...
    announceSceneSkippedFor(test) {
        ...
        this.announceSceneStartsFor(test);
        this.emit(new events_1.SceneFinishes(this.currentSceneId, outcome, this.serenity.currentTime()), new events_1.SceneFinished(this.currentSceneId, scenarioDetails, outcome, this.serenity.currentTime()));
    }
